...
def new_workout_post():
   ...
   # img = Todo(img=pic.read(),title=title, name=filename, mimetype=mimetype)
    ...
    workout = Workout(img=pic.read(),title=title, name=filename, mimetype=mimetype,pushups=pushups, comment=comment, author=current_user)
...
def update(workout_id):
    ...
    workout.complete = not workout.complete
    db.session.commit()
    return redirect(url_for('main.user_workouts'))
...
def get_img(workout_id):
    ...
    try:
        return Response(workout.img,mimetype=workout.mimetype)
